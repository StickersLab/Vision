...
function reducer(state, action) {
  ...
  switch (action.type) {
    case 'FINISH_ONBOARD': {
      const { name, weight } = action
      const weights = weight > 30 ? [...state.weights, { v: weight, date: new Date().toLocaleDateString('fr-FR',{day:'2-digit',month:'2-digit'}) }] : state.weights
      next = { ...state, name, weights }
      break
    }
    case 'TOG_CHK': {
